git  # Constants MOVE TO CONFIG.py along with MENU
MENU = [{"name": "Red Sweet", "image": "red_sweet.jpg"},
        {"name": "Green Sweet", "image": "green_sweet.jpg"},
        {"name": "Blue Sweet", "image": "blue_sweet.jpg"}
        ]

HUB = 'WayPoint20'  # Kitchen/Food source
NUMBER_OF_WAYPOINTS = 30
ONE_MACHINE = True  # Only working with LUCIE, no login for admin features
PIN = "1111"  # Not so secure Login password.
WONDERING_MODE = True  # Randomly visit WayPoints when looking for an order
TWITTER = False  # Display link to twitter or not. Use with selfie program.
STRANDS_UI_URL = "127.0.0.1:8090"  # The URL for strands__ui
